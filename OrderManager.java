...
/**
 * Manages the lists of orders in the restaurant. List of orders include:
 * current order list and completed order list.
 * 
 * @author Gregory Ong
 * @version 1.0
 * @since 2021-11-06
...
public class OrderManager {
	...
	/**
	 * Class constructor that creates the 2 lists, current orders list and the
	...
	/**
	 * Displays the Sales Revenue Report of the restaurant from a specific start
	...
	public void generateSalesRevenueReport(LocalDate startDate, LocalDate endDate) {
		...
		// Go through the completed orders list to find orders between the start and end
		// date.
		for (int i = 0; i < completedOrders.size(); i++) {
			if (completedOrders.get(i).getDatetime().compareTo(start) >= 0
					&& completedOrders.get(i).getDatetime().compareTo(end) <= 0) {
				...
				for (int j = 0; j < completedOrders.get(i).getOrderItems().size(); j++) {
					...
					if (my_dict.containsKey(name)) {
						...
						// if the order item does not exists, add it to the dictionary, including its
						// quantity
		...
		System.out.println("---------------------------------------------------------------------------");
		System.out.println();
		// Display the total revenue of the restaurant from the start date to the end
		// date.
	 ...
	 * 
	 * @param TableNumber The table number of this order.
	 * @return true if the order is billed and order invoice is printed, otherwise
	 *         return false.
	 */
	public boolean printOrderInvoice(int TableNumber) {
		...
		if (curOrder == null) {
			...
			return false;
		} else {// print order invoice(receipt) and remove this order from the current order
				// list and add it to the completed order list.
