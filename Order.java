...
/**
 * Represents an order tagged to a specific table. Each table can only have a maximum of 1 order at any point. 
 * @author Gregory Ong
 * @version 1.0
 * @since 2021-11-06
 */
public class Order {
	...
	/**
	 * Class constructor that creates an order with attributes staff, datetime, table number and customer. 
	...
	public void addItem(MenuItem item,int quantity) {
		...
		if(orderItems.size() == 0 ) orderItems.add(orderitem);
		//else search the list and update the corresponding quantity of order item if there
		// is currently the same order item in the list of order items. If not, just add it to the list of order items.
		else{
			//Go through the list of order items and try to find if there is the same order item ordered.
	 ...
	 * @param quantity The quantity of this order item that is removed from the order.
	 */
	public void removeItem(MenuItem item,int quantity) { 
		// to check if the item is found in the list of order item. 
		boolean found=false;
		// If list of order items is empty
		if(orderItems.size() == 0 ) System.out.println("There are no items in this order yet! Nothing to remove.");
		else{
			//Go through the list of order items and find the corresponding order item ordered.
			for(int i = 0; i < orderItems.size(); i++){
				//Search by name of order item.
				if(orderItems.get(i).getMenuItem().getName().equals(item.getName())){
					//If current quantity of order item in the order is less than the quantity that is pending removal from the order.
					if( (orderItems.get(i).getQuantity()-quantity) < 0 ){
						...
						//If quantity of order item after removal is zero, remove the order item from the list of order items. 
						if(orderItems.get(i).getQuantity()-quantity==0){
							orderItems.remove(i);
	...
	public void viewOrder() {
		...
		if(orderItems.size() == 0 ) System.out.println("There are no items in this order yet!");
		else{
